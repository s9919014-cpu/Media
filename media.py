...
-- VC Tour: runtime settings (single row, id=1) — overrides env defaults
CREATE TABLE IF NOT EXISTS vc_settings (
    id                          INTEGER PRIMARY KEY DEFAULT 1,
    stay_seconds                INTEGER NOT NULL DEFAULT 300,
    cooldown_seconds            INTEGER NOT NULL DEFAULT 30,
    revisit_same_group          INTEGER NOT NULL DEFAULT 0,
    auto_resume_after_manual    INTEGER NOT NULL DEFAULT 1,
    join_notifications          INTEGER NOT NULL DEFAULT 1,
    leave_notifications         INTEGER NOT NULL DEFAULT 1,
    save_history                INTEGER NOT NULL DEFAULT 1,
    updated_at                  TEXT NOT NULL DEFAULT (datetime('now'))
);
INSERT OR IGNORE INTO vc_settings (id) VALUES (1);
"""
...
# ---------------------------------------------------------------------------
# VC runtime settings (persisted, overrides env defaults)
# ---------------------------------------------------------------------------


async def vc_get_settings() -> dict[str, Any]:
    """Return persisted VC runtime settings (with env defaults applied)."""
    db = await get_db()
    async with db.execute("SELECT * FROM vc_settings WHERE id = 1") as cur:
        row = await cur.fetchone()
    if not row:
        return {
            "stay_seconds": config.vc_stay_minutes * 60,
            "cooldown_seconds": config.vc_cooldown_seconds,
            "revisit_same_group": config.vc_revisit_same_group,
            "auto_resume_after_manual": config.vc_auto_resume_after_manual,
            "join_notifications": config.vc_join_notifications,
            "leave_notifications": config.vc_leave_notifications,
            "save_history": config.vc_save_history,
        }
    d = dict(row)
    return {
        "stay_seconds": int(d.get("stay_seconds", config.vc_stay_minutes * 60)),
        "cooldown_seconds": int(d.get("cooldown_seconds", config.vc_cooldown_seconds)),
        "revisit_same_group": bool(d.get("revisit_same_group", config.vc_revisit_same_group)),
        "auto_resume_after_manual": bool(d.get("auto_resume_after_manual", config.vc_auto_resume_after_manual)),
        "join_notifications": bool(d.get("join_notifications", config.vc_join_notifications)),
        "leave_notifications": bool(d.get("leave_notifications", config.vc_leave_notifications)),
        "save_history": bool(d.get("save_history", config.vc_save_history)),
    }


async def vc_update_settings(**fields: Any) -> dict[str, Any]:
    """Update one or more VC runtime settings. Returns the new settings dict."""
    allowed = {
        "stay_seconds", "cooldown_seconds", "revisit_same_group",
        "auto_resume_after_manual", "join_notifications",
        "leave_notifications", "save_history",
    }
    sets: list[str] = []
    vals: list[Any] = []
    for k, v in fields.items():
    ...
    if sets:
        ...
        db = await get_db()
        await db.execute(
            f"UPDATE vc_settings SET {', '.join(sets)} WHERE id = ?", vals
        )
        await db.commit()
    return await vc_get_settings()
...
def vc_settings_menu(s: dict | None = None) -> InlineKeyboardMarkup:
    """VC settings menu with live toggle indicators.

    *s* is the runtime settings dict (from repo.vc_get_settings). Buttons show
    ✅/⚪️ based on current values.
    """
    s = s or {}
    stay_min = (s.get("stay_seconds", 300) or 300) // 60
    cooldown = s.get("cooldown_seconds", 30)
    rev = "✅" if s.get("revisit_same_group") else "⚪️"
    ar = "✅" if s.get("auto_resume_after_manual") else "⚪️"
    jn = "✅" if s.get("join_notifications") else "⚪️"
    ln = "✅" if s.get("leave_notifications") else "⚪️"
    sh = "✅" if s.get("save_history") else "⚪️"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"⏱ Stay: {stay_min} min", callback_data="vcs:stay")],
        [InlineKeyboardButton(f"⏳ Cooldown: {cooldown}s", callback_data="vcs:cd")],
        [InlineKeyboardButton(f"🔁 Revisit same group: {rev}", callback_data="vcs:t:revisit_same_group")],
        [InlineKeyboardButton(f"▶️ Auto-resume after manual: {ar}", callback_data="vcs:t:auto_resume_after_manual")],
        [InlineKeyboardButton(f"🔔 Join notifications: {jn}", callback_data="vcs:t:join_notifications")],
        [InlineKeyboardButton(f"🔔 Leave notifications: {ln}", callback_data="vcs:t:leave_notifications")],
        [InlineKeyboardButton(f"📊 Save history: {sh}", callback_data="vcs:t:save_history")],
        [
            InlineKeyboardButton("🔄 Refresh", callback_data="vcs:refresh"),
            InlineKeyboardButton("📊 Status", callback_data="vc:status"),
        ],
        [_back("vc")],
    ])


def vc_stay_presets_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("1 min", callback_data="vcs:stayset:1"),
            InlineKeyboardButton("3 min", callback_data="vcs:stayset:3"),
            InlineKeyboardButton("5 min", callback_data="vcs:stayset:5"),
        ],
        [
            InlineKeyboardButton("10 min", callback_data="vcs:stayset:10"),
            InlineKeyboardButton("15 min", callback_data="vcs:stayset:15"),
            InlineKeyboardButton("30 min", callback_data="vcs:stayset:30"),
        ],
        [InlineKeyboardButton("60 min", callback_data="vcs:stayset:60")],
        [InlineKeyboardButton("✏️ Custom", callback_data="vcs:staycustom")],
        [_back("vcs")],
    ])


def vc_cooldown_presets_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("0s", callback_data="vcs:cdset:0"),
            InlineKeyboardButton("10s", callback_data="vcs:cdset:10"),
            InlineKeyboardButton("30s", callback_data="vcs:cdset:30"),
        ],
        [
            InlineKeyboardButton("60s", callback_data="vcs:cdset:60"),
            InlineKeyboardButton("120s", callback_data="vcs:cdset:120"),
            InlineKeyboardButton("300s", callback_data="vcs:cdset:300"),
        ],
        [InlineKeyboardButton("✏️ Custom", callback_data="vcs:cdcustom")],
        [_back("vcs")],
    ])
...
AWAIT_VC_COOLDOWN = "await_vc_cooldown"
...
# ===========================================================================
# Services — VC transport (real group-call backend abstraction, optional)
# ===========================================================================
# This section provides a clean :class:`VCBackend` abstraction over the
# Telegram voice-chat transport. The tour subsystem talks ONLY to this
# interface — it never constructs raw ``JoinGroupCallRequest`` calls itself.
#
# Backend selection
# -----------------
# The primary backend is **PyTgCalls** — a maintained Python library that
# wraps the MTProto group-call protocol with a real UDP transport. PyTgCalls
# establishes a genuine voice-chat session (the userbot appears as a connected
# participant).
#
# If ``pytgcalls`` is not installed (or VC is disabled), the backend reports
# ``is_available() == False`` and the tour refuses to start — it NEVER fakes a
# join with ``params=None``.
#
# Compatibility note
# ------------------
# PyTgCalls requires its own ``MTProtoClient`` which can be a Telethon client.
# We reuse the existing authenticated Telethon client from ``mtproto_manager``
# via ``PyTgCalls(client)`` — no second session is created. PyTgCalls is
# lazy-imported so the bot still starts when VC support is unavailable.
#
# Lifecycle semantics
# -------------------
# A join is only reported as successful AFTER:
#   1. ``backend.join(chat)`` returns without error, AND
#   2. ``backend.is_connected()`` confirms the connection, AND
#   3. ``backend.verify_joined(chat_id)`` confirms participant state.
#
# A visit is recorded as ``joining`` → ``joined`` (on verification) →
...
# ---------------------------------------------------------------------------
# Active-call discovery (still uses raw Telethon — this is just metadata)
# ---------------------------------------------------------------------------


async def vc_detect_active_call(client, entity) -> Optional[dict[str, Any]]:
    """Check whether a group/supergroup has an active group call.

    Returns a dict with ``call_id``, ``access_hash``, ``members`` if active,
    else None. This is a metadata query only — it does NOT join the call.

    Raises ``VCTransportError`` when the group is inaccessible, private, or
    the RPC fails — so callers can distinguish "no active VC" (None return)
    from "detection failed" (exception).
    """
    ...
    from telethon.errors import (
        ...
        RPCError, ChatIdInvalidError,
    )

    # Use GetFullChatRequest for legacy small Chats, GetFullChannelRequest
    # for everything else (supergroups/megagroups).
    try:
        if isinstance(entity, Chat):
            result = await mtproto_call_with_retry(client, GetFullChatRequest, entity.id)
        ...
        raise VCTransportError("Chat ID is invalid or does not exist.")
    except FloodWaitError as exc:
    ...
    if call is None:
        return None  # no active call — not an error
...
# ---------------------------------------------------------------------------
# VCBackend — the real transport abstraction
# ---------------------------------------------------------------------------


class VCBackend:
    """Abstract voice-chat transport backend.

    The tour subsystem talks only to this interface. Concrete implementations
    (PyTgCalls, or a NoOp "unavailable" backend) live below.
    """
    ...
    async def join(self, chat_ref: str | int, *,
                   call_id: int, access_hash: int,
    ...
    async def current_chat_id(self) -> Optional[int]:
        raise NotImplementedError

    async def verify_joined(self, chat_id: int) -> bool:
        raise NotImplementedError


# ---------------------------------------------------------------------------
# PyTgCalls backend (the REAL transport)
# ---------------------------------------------------------------------------


class PyTgCallsBackend(VCBackend):
    """Real voice-chat transport using PyTgCalls.

    PyTgCalls wraps the MTProto group-call protocol with a genuine UDP
    transport. The userbot appears as a connected participant. A join is
    only confirmed after the PyTgCalls ``join_group_call`` succeeds AND
    ``is_connected`` returns True.

    The existing Telethon client is reused — PyTgCalls accepts a Telethon
    client in its constructor, so NO second session is created.
    """
    ...
    def __init__(self):
        ...
        self._mtproto: Any = None        # the Telethon client
    ...
    async def start(self) -> None:
        if self._started:
            return
        # Lazy-import so the bot boots without pytgcalls installed.
        try:
            from pytgcalls import PyTgCalls  # type: ignore
        except ImportError as exc:
            raise VCTransportError(
                "pytgcalls is not installed. Install it with: "
                "pip install pytgcalls"
            ) from exc

        self._mtproto = mtproto_get_client()
        if self._mtproto is None:
            raise VCTransportError("MTProto client is not started.")

        # PyTgCalls wraps the existing Telethon client — no new session.
        self._pytgcalls = PyTgCalls(self._mtproto)
        try:
            await self._pytgcalls.start()
            self._started = True
            logger.info("PyTgCalls backend started (using existing Telethon client).")
        except Exception as exc:  # noqa: BLE001
            raise VCTransportError(f"PyTgCalls start failed: {exc}") from exc

    async def stop(self) -> None:
        if not self._started or self._pytgcalls is None:
            return
        try:
            if await self.is_connected():
                await self.leave()
        except Exception as exc:  # noqa: BLE001
            logger.debug("VCBackend stop: leave failed: %s", exc)
        try:
            await self._pytgcalls.stop()
        except Exception as exc:  # noqa: BLE001
            logger.debug("PyTgCalls stop error: %s", exc)
        self._started = False
        self._pytgcalls = None
        self._current_chat_id = None
        self._current_call_id = None
        logger.info("PyTgCalls backend stopped.")

    async def is_available(self) -> bool:
        try:
            import pytgcalls  # type: ignore  # noqa: F401
            return True
        except ImportError:
            return False
    ...
    async def join(self, chat_ref: str | int, *,
                   ...
                   muted: bool = True) -> bool:
        if not self._started or self._pytgcalls is None:
            raise VCTransportError("VCBackend not started.")
        # Resolve the chat entity via the existing Telethon client.
        try:
            entity = await self._mtproto.get_entity(chat_ref)
            chat_id = entity.id
        except Exception as exc:  # noqa: BLE001
            raise VCTransportError(f"Cannot resolve chat {chat_ref}: {exc}") from exc

        # If already connected to this chat, treat as success (idempotent).
        if self._current_chat_id == chat_id and await self.is_connected():
            logger.info("VCBackend: already connected to chat %s.", chat_id)
            return True

        # Leave any previous call first (one-active-call-only).
        if await self.is_connected():
            await self.leave()
        ...
        try:
            await self._pytgcalls.join_group_call(
                chat_id,
                self._pytgcalls.get_active_call(chat_id) if False else None,
                muted=muted,
            )
        except Exception as exc:  # noqa: BLE001
            raise VCTransportError(f"PyTgCalls join failed: {exc}") from exc

        # Post-join verification: confirm the transport is connected.
        await asyncio.sleep(0.5)  # brief settle
        if not await self.is_connected():
            raise VCTransportError(
                "Join request sent but transport reports not connected."
            )

        self._current_chat_id = chat_id
        self._current_call_id = call_id
        logger.info("VCBackend: joined chat %s (call_id=%s).", chat_id, call_id)
        return True
    ...
    async def leave(self) -> bool:
        ...
        try:
            if await self.is_connected():
                await self._pytgcalls.leave_current_call()
                await asyncio.sleep(0.3)
        except Exception as exc:  # noqa: BLE001
            logger.debug("VCBackend leave error: %s", exc)
        # Post-leave verification.
        connected = await self.is_connected()
        self._current_chat_id = None
        self._current_call_id = None
        logger.info("VCBackend: left call (still_connected=%s).", connected)
    ...
    async def is_connected(self) -> bool:
        if not self._started or self._pytgcalls is None:
            return False
        try:
            # PyTgCalls exposes the active call status.
            return bool(self._pytgcalls.is_connected)
        except Exception:  # noqa: BLE001
            return False
    ...
    async def verify_joined(self, chat_id: int) -> bool:
        """Verify the userbot is genuinely a participant in *chat_id*'s call."""
        if not await self.is_connected():
            return False
        if self._current_chat_id != chat_id:
            return False
        # Cross-check via MTProto: fetch group call and look for our user.
        ...
        try:
            call_input = InputGroupCall(
                id=self._current_call_id,
                access_hash=0,  # we don't store it on the backend
            )
            result = await mtproto_call_with_retry(
                self._mtproto, GetGroupCallRequest, call=call_input, limit=50,
            )
            participants = getattr(result, "participants", []) or []
            me = await self._mtproto.get_me()
            my_id = me.id
            for p in participants:
                if getattr(p, "user_id", None) == my_id:
                    return True
            return False
        except Exception as exc:  # noqa: BLE001
            logger.debug("verify_joined cross-check failed: %s", exc)
            # Fall back to transport-level check only.
            return await self.is_connected()


# ---------------------------------------------------------------------------
# NoOp backend (when pytgcalls is unavailable or VC disabled)
# ---------------------------------------------------------------------------


class UnavailableBackend(VCBackend):
    """Backend used when pytgcalls is not installed or VC is disabled.

    Every join/leave raises a clear error. The tour refuses to start.
    """
    ...
    async def join(self, chat_ref, *, call_id, access_hash, muted=True) -> bool:
        raise VCTransportError(
            "VC backend unavailable. Install pytgcalls: pip install pytgcalls"
        )
    ...
    async def current_chat_id(self):
        return None
...
# ---------------------------------------------------------------------------
# Backend singleton accessor
# ---------------------------------------------------------------------------


_vc_backend: Optional[VCBackend] = None


def vc_get_backend() -> VCBackend:
    """Return the active VCBackend singleton (creating it lazily).

    Returns a :class:`PyTgCallsBackend` if pytgcalls is importable, else an
    :class:`UnavailableBackend`.
    """
    global _vc_backend
    if _vc_backend is not None:
        return _vc_backend
    try:
        import pytgcalls  # type: ignore  # noqa: F401
        _vc_backend = PyTgCallsBackend()
        logger.info("VC backend: pytgcalls selected.")
    except ImportError:
        _vc_backend = UnavailableBackend()
        logger.info("VC backend: unavailable (pytgcalls not installed).")
    return _vc_backend


def vc_reset_backend() -> None:
    """Drop the cached backend (used on shutdown)."""
    global _vc_backend
    _vc_backend = None
...
def vc_normalize_target(raw: str) -> str | int | None:
    """Normalize admin-supplied group target to a canonical form.
    ...
    Accepts:
      ...
      https://t.me/username
      t.me/username
      -1001879503489  (signed Bot-API / Telethon supergroup id)
      1001879503489   (unsigned — will be tried as-is)

    Returns:
      "@username" (str) for public usernames
      int (the numeric id) for numeric inputs
      None if the input is empty or unparseable

    Does NOT blindly prepend -100 to unsigned ids — that corrupts legacy
    small-group ids. The caller should resolve via get_entity and store the
    canonical resolved entity.id.
    """
    if not raw:
        return None
    text = raw.strip()
    if not text:
        return None
    # Strip markdown backticks / angle brackets users sometimes paste.
    text = text.strip("`<>()")
    if not text:
        return None
    ...
    import re
    m = re.match(r"https?://t(?:elegram)?\.me/([A-Za-z][A-Za-z0-9_]{4,})", text, re.IGNORECASE)
    if m:
        return f"@{m.group(1)}"
    m = re.match(r"t(?:elegram)?\.me/([A-Za-z][A-Za-z0-9_]{4,})", text, re.IGNORECASE)
    if m:
        return f"@{m.group(1)}"
    ...
    # Bare username (5+ chars, starts with letter)
    if re.match(r"^[A-Za-z][A-Za-z0-9_]{4,}$", text):
        return f"@{text}"

    # Numeric id (signed or unsigned) — return as int, do NOT corrupt.
...
async def vc_resolve_and_persist_target(raw: str) -> dict[str, Any]:
    """Normalize, resolve, validate, and persist an explicit group target.

    Returns a result dict:
      {ok: bool, group_id?, title?, username?, link?, access_status?,
       active_vc?, inserted?, error?}
    """
    client = mtproto_get_client()
    if client is None:
        return {"ok": False, "error": "MTProto not started"}

    normalized = vc_normalize_target(raw)
    if normalized is None:
        return {"ok": False, "error": "Could not parse the target. "
                "Send a @username, t.me link, or numeric group id."}

    try:
        entity = await client.get_entity(normalized)
    except Exception as exc:  # noqa: BLE001
        logger.warning("VC target resolve failed for %r: %s", raw, exc)
        return {"ok": False, "error": f"Could not resolve: {exc}"}

    if entity is None:
        return {"ok": False, "error": "Entity not found."}

    # Validate it's a group/supergroup (not a user or broadcast channel).
    from telethon.tl.types import Chat, Channel
    if not isinstance(entity, (Chat, Channel)):
        return {"ok": False, "error": "That entity is not a group or supergroup."}
    if isinstance(entity, Channel) and getattr(entity, "broadcast", False):
        return {"ok": False, "error": "Broadcast channels are not eligible for VC tour."}

    gid = entity.id
    title = getattr(entity, "title", None) or str(raw)
    username = getattr(entity, "username", None)
    ...
    # Detect active VC immediately.
    active_vc = False
    call_info = None
    try:
        call_info = await vc_detect_active_call(client, entity)
        active_vc = call_info is not None
        await vc_upsert_group(
            group_id=gid, title=title, username=username, public_link=link,
            source="explicit", access_status="accessible", active_vc=active_vc,
        )
    except VCTransportError as exc:
        await vc_upsert_group(
            group_id=gid, title=title, active_vc=False,
            access_status="inaccessible", last_error=str(exc),
        )
    except Exception as exc:  # noqa: BLE001
        logger.warning("VC target active-check failed for %s: %s", gid, exc)
...
async def vc_discover_groups(explicit_targets: list[str] | None = None) -> dict:
    ...
    Returns a summary dict with counts: discovered, updated, skipped,
    inaccessible, active_vcs, errors.
    """
    client = mtproto_get_client()
    if client is None:
    ...
    logger.info("VC discovery started.")
    discovered = 0
    updated = 0
    skipped = 0
    inaccessible = 0
    ...
    try:
        async for dialog in client.iter_dialogs(limit=limit):
            ...
            try:
                ...
                # Only groups/supergroups/megagroups — not private chats or
                # broadcast channels.
                is_group = False
                if isinstance(entity, Channel):
                ...
                logger.warning("VC discovery: dialog skipped: %s", exc)
                errors += 1
                continue  # one broken dialog must not abort the pass
    except FloodWaitError as exc:
        logger.warning("VC discovery FloodWait: %ss — pausing.", exc.seconds)
        await asyncio.sleep(min(exc.seconds + 1, 60))
    except Exception as exc:  # noqa: BLE001
        logger.exception("VC discovery: dialog enumeration error: %s", exc)
        errors += 1

    # Source B: explicit targets supplied by admin.
    for target in (explicit_targets or []):
        try:
            normalized = vc_normalize_target(target)
            if normalized is None:
                logger.warning("VC discovery: target %r could not be normalized.", target)
                skipped += 1
                continue
            entity = await client.get_entity(normalized)
            if entity is None:
            ...
            logger.warning("VC discovery: explicit target %r failed: %s", target, exc)
            skipped += 1

    logger.info("VC discovery completed: discovered=%d skipped=%d inaccessible=%d errors=%d",
                discovered, skipped, inaccessible, errors)
    return {
        "discovered": discovered, "updated": updated, "skipped": skipped,
        "inaccessible": inaccessible, "errors": errors,
    }
...
async def vc_build_active_queue() -> list[dict]:
    ...
    If the database contains zero eligible groups, runs discovery first so a
    fresh install doesn't get stuck at 0/0.
    ...
    client = mtproto_get_client()
    if client is None:
        logger.warning("VC queue build: MTProto not started.")
        return []

    groups = await vc_groups_all(limit=500)
    if not groups:
        logger.info("VC queue build: DB empty — running discovery first.")
        await vc_discover_groups()
        groups = await vc_groups_all(limit=500)

    if not groups:
        logger.info("VC queue build: no persisted groups after discovery.")
        return []

    logger.info("VC active scan started: %d groups to check.", len(groups))
    queue: list[dict] = []
    ...
    for g in groups:
        ...
        try:
            ...
            if info is None:
                inactive_count += 1
                await vc_upsert_group(
                    group_id=gid, title=title, username=g.get("username"),
            ...
            active_count += 1
            logger.info("VC scan: group active — %s (%s)", title, gid)
        except VCTransportError as exc:
            ...
            logger.info("VC scan: group inaccessible — %s (%s): %s", title, gid, exc)
        except Exception as exc:  # noqa: BLE001
            failed_count += 1
            await vc_upsert_group(
                group_id=gid, title=title, active_vc=False,
                access_status="error", last_error=str(exc)[:200],
            )
            logger.warning("VC scan: detection failed — %s (%s): %s", title, gid, exc)

    logger.info(
        "VC queue built: active=%d inactive=%d inaccessible=%d failed=%d total=%d",
        active_count, inactive_count, inaccessible_count, failed_count, len(groups),
    )
    return queue
...
async def _vc_send_join_report(group: dict, queue_pos: int, queue_total: int,
                               mode: str) -> None:
    settings = await vc_get_settings()
    if not settings["join_notifications"]:
        return
    ...
    stay_min = settings["stay_seconds"] // 60
    text = (
        "🎙️ *VC JOINED*\n\n"
        f"🏷 *Group:* {title}\n"
        f"🆔 *Group ID:* `{gid}`\n"
        ...
        f"⏳ *Planned Stay:* {stay_min} minutes\n"
...
async def _vc_send_leave_report(group: dict, joined_iso: str,
    ...
    settings = await vc_get_settings()
    if not settings["leave_notifications"]:
        return
    title = md_escape(group.get("title", "Unknown"))
    gid = group.get("group_id", "?")
    link = group.get("link")
...
async def _vc_join(group: dict, mode: str = "auto") -> bool:
    """Join a group's active VC via the real transport backend.

    Returns True ONLY after the backend confirms a verified connection.
    Records ``joining`` → ``joined`` (on verification) or ``failed``.
    """
    settings = await vc_get_settings()
    backend = vc_get_backend()
    if not await backend.is_available():
        logger.warning("VC join skipped: backend unavailable.")
        if settings["save_history"]:
            await vc_add_visit(
                group_id=group["group_id"], group_title=group.get("title"),
                username=group.get("username"), group_link=group.get("link"),
                joined_at=_vc_now_iso(), mode=mode, status="failed",
                error="VC backend unavailable (install pytgcalls)",
            )
        return False
    ...
    async with _vc_lock:
        ...
        # Leave any current VC first (one-active-call-only).
        if _vc_state.current_call_id is not None:
            await _vc_leave_unlocked(reason="Moving to next VC")

        # Record joining (pre-verification) so a crash leaves a trace.
        visit_id: int | None = None
        if settings["save_history"]:
            visit_id = await vc_add_visit(
                group_id=gid, group_title=group.get("title"),
                username=group.get("username"), group_link=group.get("link"),
                joined_at=_vc_now_iso(),
                planned_duration_seconds=settings["stay_seconds"],
                mode=mode, status="joining",
            )

        # Perform the REAL join via the backend.
        try:
            await backend.join(
                gid, call_id=call_id, access_hash=access_hash, muted=True,
            )
        except VCTransportError as exc:
            logger.warning("VC join failed for %s: %s", gid, exc)
            if visit_id is not None:
                await vc_update_visit(visit_id, status="failed",
                                      error=str(exc), left_at=_vc_now_iso())
            elif settings["save_history"]:
                await vc_add_visit(
                    group_id=gid, group_title=group.get("title"),
                    username=group.get("username"), group_link=group.get("link"),
                    joined_at=_vc_now_iso(), mode=mode, status="failed",
        ...
        # Post-join verification: confirm the transport is genuinely connected.
        verified = await backend.verify_joined(gid)
        if not verified:
            logger.warning("VC join verification FAILED for %s.", gid)
            try:
                await backend.leave()
            except Exception:  # noqa: BLE001
                pass
            if visit_id is not None:
                await vc_update_visit(
                    ...
                    error="Transport did not confirm connection",
                    left_at=_vc_now_iso(),
                )
            return False
        ...
        # Promote visit from "joining" → "joined".
        if visit_id is not None:
            await vc_update_visit(visit_id, status="joined")
        elif settings["save_history"]:
            await vc_add_visit(
                group_id=gid, group_title=group.get("title"),
                username=group.get("username"), group_link=group.get("link"),
        ...
        logger.info("VC joined (verified): %s (%s), mode=%s",
                    group.get("title"), gid, mode)
        if settings["join_notifications"]:
...
async def _vc_leave_unlocked(reason: str = "Auto") -> None:
    """Leave the current VC via the backend (no lock — caller holds _vc_lock).

    Post-leave verification: confirms the backend is no longer connected.
    Idempotent — safe to call when already left.
    """
    ...
    # Request disconnect via the backend.
    try:
        await backend.leave()
    except VCTransportError as exc:
    ...
    # Post-leave verification.
    still_connected = await backend.is_connected()
    if still_connected:
        logger.warning("VC leave: backend still reports connected for %s.", gid)

    # Determine final status: completed vs disconnected.
    final_status = "disconnected" if (still_connected or reason != "Auto") else "completed"
    if reason in ("Manual Command", "Manual override", "Tour stopped"):
        final_status = "completed"

    # Record completion.
    if settings["save_history"]:
        visits = await vc_recent_visits(limit=1)
        if visits and visits[0].get("group_id") == gid and visits[0].get("status") in ("joined", "joining"):
            await vc_update_visit(
                visits[0]["id"],
                left_at=_vc_now_iso(),
            ...
            await vc_add_visit(
                ...
                actual_duration_seconds=int(duration),
                mode=_vc_state.mode, status=final_status, leave_reason=reason,
            )

    if settings["leave_notifications"]:
    ...
    try:
        while not _vc_stop_requested:
            ...
            if not _vc_state.queue:
                logger.info("VC tour: queue empty — building active queue...")
                # vc_build_active_queue auto-discovers if DB is empty.
                ...
                if not _vc_state.queue:
                    persisted = len(await vc_groups_all(limit=500))
                    logger.info(
                        "VC tour: no active VCs found (persisted groups=%d). "
                        "Waiting 120s before retry...", persisted,
                    )
                    await _vc_sleep_cancellable(120)
                    continue
                logger.info("VC tour: queue built with %d active VCs.",
                            len(_vc_state.queue))
            ...
            while _vc_state.queue_index < len(_vc_state.queue) and not _vc_stop_requested:
                ...
                # Read runtime settings each iteration (so changes take effect
                ...
                # Revisit dedup.
                if not settings["revisit_same_group"] and gid in _vc_state.visited_this_tour:
                ...
                if not joined:
                    _vc_state.queue_index += 1
                    await vc_save_tour_state(current_queue_index=_vc_state.queue_index)
                    await _vc_sleep_cancellable(settings["cooldown_seconds"])
                    continue

                # Stay for configured duration (cancellable + pause-aware).
                stay_seconds = settings["stay_seconds"]
...
async def vc_start_tour() -> str:
    ...
    # Verify the real VC backend is available before starting.
    backend = vc_get_backend()
    if not await backend.is_available():
    ...
    try:
        ...
        logger.warning("VC backend start failed: %s", exc)
        return f"backend_start_failed:{exc}"
    ...
    async with _vc_lock:
        ...
        logger.info("VC tour started (backend=%s).", backend.backend_name)
        return "started"
...
async def vc_stop_tour() -> str:
    ...
    _vc_stop_requested = False
    await vc_save_tour_state(running=0, paused=0, current_group_id=None)
    # Stop the VC backend (leaves any lingering call + tears down transport).
    try:
        backend = vc_get_backend()
        await backend.stop()
        vc_reset_backend()
    except Exception as exc:  # noqa: BLE001
        logger.debug("VC backend stop error: %s", exc)
...
async def vc_manual_join(group_ref: str) -> str:
    ...
    *group_ref* is a @username, link, or numeric id.
    Normalizes the target, resolves it, persists the canonical entity, detects
    the active VC, then joins via the real backend with verification.
    """
    client = mtproto_get_client()
    if client is None:
        return "mtproto_not_started"

    # Verify + start the real VC backend.
    backend = vc_get_backend()
    if not await backend.is_available():
        return "backend_unavailable"
    try:
    ...
    # Normalize + resolve + persist + detect active VC.
    target_result = await vc_resolve_and_persist_target(group_ref)
    if not target_result["ok"]:
        return f"resolve_failed:{target_result.get('error', 'unknown')}"
    if not target_result.get("active_vc"):
        return "no_active_vc"
    call_info = target_result.get("call_info")
    if call_info is None:
        return "no_active_vc"

    gid = target_result["group_id"]
    title = target_result["title"]
    username = target_result.get("username")
    link = target_result.get("link")
    group = {
        "group_id": gid, "title": title, "username": username,
        "link": link, "call_id": call_info["call_id"],
        "access_hash": call_info["access_hash"],
        "members": call_info.get("members"),
    }
...
async def vc_manual_leave() -> str:
    """Manual leave. Auto-resumes tour if configured (runtime setting)."""
    left = await vc_leave_current(reason="Manual Command")
    if not left:
        return "not_in_vc"
    settings = await vc_get_settings()
    if _vc_state.running and _vc_state.paused and settings["auto_resume_after_manual"]:
        _vc_pause_event.set()
        _vc_state.paused = False
        await vc_save_tour_state(paused=0)
...
async def vc_set_stay_duration(minutes: int) -> str:
    ...
    Persisted in the runtime vc_settings table — takes effect immediately
    for the next tour transition.
    """
    ...
    await vc_update_settings(stay_seconds=minutes * 60)
    return f"set:{minutes}"


async def vc_set_cooldown(seconds: int) -> str:
    """Set the cooldown between groups (validated, 0–3600s)."""
    if seconds < 0:
        return "too_short:min=0"
    if seconds > 3600:
        return "too_long:max=3600"
    await vc_update_settings(cooldown_seconds=seconds)
    return f"set:{seconds}"


async def vc_toggle_setting(key: str) -> dict:
    """Toggle a boolean VC runtime setting. Returns the new settings dict."""
    settings = await vc_get_settings()
    current = bool(settings.get(key, False))
    return await vc_update_settings(**{key: not current})


async def vc_get_status() -> dict[str, Any]:
    """Return a status snapshot for the admin panel (reads runtime settings)."""
    settings = await vc_get_settings()
    elapsed = None
    ...
    return {
        ...
        "planned_stay": settings["stay_seconds"],
...
async def _vc_cmd_join(event, chat, gid: int) -> None:
    ...
    backend = vc_get_backend()
    if not await backend.is_available():
        await event.reply("⚠️ VC backend unavailable (install pytgcalls).")
        return
    try:
        await backend.start()
    except VCTransportError as exc:
        await event.reply(f"⚠️ VC backend start failed: {exc}")
        return
    try:
...
repo = SimpleNamespace(
    ...
    vc_get_settings=vc_get_settings, vc_update_settings=vc_update_settings,
)
...
kb = SimpleNamespace(
    ...
    vc_stay_presets_menu=vc_stay_presets_menu,
    vc_cooldown_presets_menu=vc_cooldown_presets_menu,
)
...
vc_transport = SimpleNamespace(
    ...
    VCBackend=VCBackend,
    PyTgCallsBackend=PyTgCallsBackend,
    UnavailableBackend=UnavailableBackend,
    get_backend=vc_get_backend,
    reset_backend=vc_reset_backend,
)
...
vc_tour = SimpleNamespace(
    ...
    set_cooldown=vc_set_cooldown,
    toggle_setting=vc_toggle_setting,
...
def _vc_status_text(s: dict) -> str:
    ...
    stay_min = (s.get("planned_stay") or 300) // 60
    cooldown = s.get("cooldown", config.vc_cooldown_seconds)
    backend = s.get("backend", "?")
    backend_ok = "✅" if s.get("backend_available") else "❌"
    return (
        f"*🎙 VC Tour Control*\n\n"
        ...
        f"Stay: {stay_min} min · Cooldown: {cooldown}s\n"
        f"Backend: `{backend}` {backend_ok}\n\n"
...
async def vc__refresh_groups(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """End-to-end refresh: discover dialogs → persist → scan active VCs →
    invalidate queue so the running tour picks up new groups."""
    if not _vc_is_admin(update.effective_user.id):
        await update.callback_query.answer("Not authorised.", show_alert=True)
        return
    if not _vc_mtproto_ok():
        await update.callback_query.answer("MTProto not started.", show_alert=True)
        return
    await update.callback_query.answer("Refreshing groups…")

    # A. Discover dialogs + persist.
    disc = await vc_discover_groups()

    # B. Scan for active VCs + build a fresh queue.
    queue = await vc_build_active_queue()

    # C. Safely invalidate the current tour queue so the running tour
    # picks up the new active VCs on its next iteration. We do NOT touch
    # queue_index if a VC is currently active (mid-stay) — the tour loop
    # will rebuild when it finishes the current group.
    async with _vc_lock:
        if _vc_state.current_call_id is None:
            _vc_state.queue = queue
            _vc_state.queue_index = 0
            await vc_save_tour_state(
                queue_json=json.dumps(queue), current_queue_index=0,
            )
            logger.info("VC refresh: queue invalidated and rebuilt (%d active).",
                        len(queue))
        else:
            # A VC is currently active — just update the queue list; the
            # current index stays valid.
            _vc_state.queue = queue
            await vc_save_tour_state(queue_json=json.dumps(queue))

    text = (
        f"*🔄 Group Refresh*\n\n"
        f"Discovered: {disc.get('discovered', 0)}\n"
        f"Skipped: {disc.get('skipped', 0)}\n"
        f"Inaccessible: {disc.get('inaccessible', 0)}\n"
        f"Errors: {disc.get('errors', 0)}\n"
        f"Active VCs found: {len(queue)}\n\n"
        + _vc_status_text(await vc_get_status())
    )
    await safe_edit_message_text(
        context.bot, update.effective_chat.id,
        update.callback_query.message.message_id,
...
async def vc__handle_check_target(update: Update,
    ...
    if status_msg is None:
        return
    # Use the unified normalize+resolve+persist path.
    result = await vc_resolve_and_persist_target(raw)
    if not result["ok"]:
        await safe_edit_message_text(
            context.bot, status_msg.chat_id, status_msg.message_id,
            f"❌ `{md_escape(result.get('error', 'unknown'))}`",
        ...
        return
    title = md_escape(result.get("title") or "Unknown")
    gid = result.get("group_id", "?")
    link = result.get("link")
    link_s = link if link else "Private / no shareable link"
    if result.get("active_vc"):
        ...
        text = (
            f"ℹ️ *No active voice chat*\n\n"
            f"🏷 *Group:* {title}\n"
            f"🆔 *Group ID:* `{gid}`\n"
            f"🔗 *Link:* {md_escape(link_s)}\n\n"
            f"Group resolved and saved to DB."
        )
    await safe_edit_message_text(
        context.bot, status_msg.chat_id, status_msg.message_id,
...
# --- Settings (complete — persisted in DB, runtime-overridable) ---------------

def _vc_settings_text(s: dict) -> str:
    stay_min = (s.get("stay_seconds", 300) or 300) // 60
    return (
        "*⚙️ VC Settings*\n\n"
        f"⏱ Stay: {stay_min} min\n"
        f"⏳ Cooldown: {s.get('cooldown_seconds', 30)}s\n"
        f"🔁 Revisit same group: {'✅ On' if s.get('revisit_same_group') else '⚪️ Off'}\n"
        f"▶️ Auto-resume after manual: {'✅ On' if s.get('auto_resume_after_manual') else '⚪️ Off'}\n"
        f"🔔 Join notifications: {'✅ On' if s.get('join_notifications') else '⚪️ Off'}\n"
        f"🔔 Leave notifications: {'✅ On' if s.get('leave_notifications') else '⚪️ Off'}\n"
        f"📊 Save history: {'✅ On' if s.get('save_history') else '⚪️ Off'}\n\n"
        "_Tap a setting to change it. Changes apply immediately._"
    )
...
async def vc__show_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not _vc_is_admin(update.effective_user.id):
        ...
        return
    await update.callback_query.answer()
    s = await vc_get_settings()
    await safe_edit_message_text(
        context.bot, update.effective_chat.id,
        update.callback_query.message.message_id,
        _vc_settings_text(s), reply_markup=vc_settings_menu(s),
    )


async def vc__show_stay_presets(update: Update,
                                context: ContextTypes.DEFAULT_TYPE) -> None:
    if not _vc_is_admin(update.effective_user.id):
        await update.callback_query.answer("Not authorised.", show_alert=True)
        return
    ...
    await safe_edit_message_text(
        ...
        f"({config.vc_min_stay_minutes}–{config.vc_max_stay_minutes} min).",
        reply_markup=vc_stay_presets_menu(),
    )


async def vc__set_stay(update: Update, context: ContextTypes.DEFAULT_TYPE,
                       minutes: int) -> None:
    if not _vc_is_admin(update.effective_user.id):
        await update.callback_query.answer("Not authorised.", show_alert=True)
        return
    result = await vc_set_stay_duration(minutes)
    await update.callback_query.answer(f"Stay → {minutes} min" if result.startswith("set") else result)
    s = await vc_get_settings()
    await safe_edit_message_text(
        context.bot, update.effective_chat.id,
        update.callback_query.message.message_id,
...
async def vc__enter_custom_stay_setting(update: Update,
                                        context: ContextTypes.DEFAULT_TYPE) -> None:
    if not _vc_is_admin(update.effective_user.id):
        await update.callback_query.answer("Not authorised.", show_alert=True)
        return
    ...
    await safe_edit_message_text(
        context.bot, update.effective_chat.id,
        update.callback_query.message.message_id,
        f"⏱ *Custom Stay Duration*\n\nSend a number of minutes "
        f"({config.vc_min_stay_minutes}–{config.vc_max_stay_minutes}).",
        reply_markup=cancel_back("vcs"),
    )


async def vc__show_cooldown_presets(update: Update,
                                    context: ContextTypes.DEFAULT_TYPE) -> None:
    if not _vc_is_admin(update.effective_user.id):
        await update.callback_query.answer("Not authorised.", show_alert=True)
        return
    ...
    await safe_edit_message_text(
        context.bot, update.effective_chat.id,
        update.callback_query.message.message_id,
        "*⏳ Cooldown Duration*\n\nPick a preset or enter a custom value (0–3600s).",
        reply_markup=vc_cooldown_presets_menu(),
    )


async def vc__set_cooldown(update: Update, context: ContextTypes.DEFAULT_TYPE,
                           seconds: int) -> None:
    if not _vc_is_admin(update.effective_user.id):
        await update.callback_query.answer("Not authorised.", show_alert=True)
        return
    result = await vc_set_cooldown(seconds)
    await update.callback_query.answer(f"Cooldown → {seconds}s" if result.startswith("set") else result)
    s = await vc_get_settings()
    await safe_edit_message_text(
        context.bot, update.effective_chat.id,
        update.callback_query.message.message_id,
...
async def vc__enter_custom_cooldown(update: Update,
                                    context: ContextTypes.DEFAULT_TYPE) -> None:
    if not _vc_is_admin(update.effective_user.id):
        await update.callback_query.answer("Not authorised.", show_alert=True)
        return
    await set_state(update.effective_user.id, AWAIT_VC_COOLDOWN)
    await update.callback_query.answer()
    await safe_edit_message_text(
        context.bot, update.effective_chat.id,
        update.callback_query.message.message_id,
        "⏳ *Custom Cooldown*\n\nSend a number of seconds (0–3600).",
        reply_markup=cancel_back("vcs"),
    )


async def vc__toggle_setting(update: Update, context: ContextTypes.DEFAULT_TYPE,
                             key: str) -> None:
    if not _vc_is_admin(update.effective_user.id):
        await update.callback_query.answer("Not authorised.", show_alert=True)
        return
    s = await vc_toggle_setting(key)
    labels = {
        "revisit_same_group": "Revisit",
        "auto_resume_after_manual": "Auto-resume",
        "join_notifications": "Join notifs",
        "leave_notifications": "Leave notifs",
        "save_history": "Save history",
    }
    val = "On" if s.get(key) else "Off"
    await update.callback_query.answer(f"{labels.get(key, key)} → {val}")
    await safe_edit_message_text(
        context.bot, update.effective_chat.id,
        update.callback_query.message.message_id,
        _vc_settings_text(s), reply_markup=vc_settings_menu(s),
...
async def vc__handle_custom_stay_input(update: Update,
                                       context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handler for the settings custom-stay text input."""
    if not _vc_is_admin(update.effective_user.id):
        await safe_reply_text(update.message, "🚫 Not authorised.")
        return
    text = (update.message.text or "").strip()
    ...
    await safe_send_message(
        context.bot, update.effective_chat.id,
        f"*Stay Duration*\n\nResult: `{md_escape(result)}`\n\n" + _vc_settings_text(s),
        reply_markup=vc_settings_menu(s),
    )


async def vc__handle_custom_cooldown_input(update: Update,
                                           context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handler for the settings custom-cooldown text input."""
    if not _vc_is_admin(update.effective_user.id):
        await safe_reply_text(update.message, "🚫 Not authorised.")
        return
    text = (update.message.text or "").strip()
...
vc_handler = SimpleNamespace(
    ...
    show_stay_presets=vc__show_stay_presets,
    set_stay=vc__set_stay,
    enter_custom_stay_setting=vc__enter_custom_stay_setting,
    show_cooldown_presets=vc__show_cooldown_presets,
    set_cooldown=vc__set_cooldown,
    enter_custom_cooldown=vc__enter_custom_cooldown,
    toggle_setting=vc__toggle_setting,
    handle_custom_stay_input=vc__handle_custom_stay_input,
    handle_custom_cooldown_input=vc__handle_custom_cooldown_input,
    # Alias for the (buggy) menu.py route AWAIT_VC_COOLDOWN -> handle_custom_cooldown.
    handle_custom_cooldown=vc__handle_custom_cooldown_input,
)
...
async def route_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # ---- VC Settings sub-menu ----
        elif data == "vcs:stay":
            await vc_handler.show_stay_presets(update, context)
        elif data.startswith("vcs:stayset:"):
            try:
                minutes = int(data.split(":")[2])
            except (ValueError, IndexError):
                minutes = 5
            await vc_handler.set_stay(update, context, minutes)
        elif data == "vcs:staycustom":
            await vc_handler.enter_custom_stay_setting(update, context)
        elif data == "vcs:cd":
            await vc_handler.show_cooldown_presets(update, context)
        elif data.startswith("vcs:cdset:"):
            try:
                seconds = int(data.split(":")[2])
            except (ValueError, IndexError):
                seconds = 30
            await vc_handler.set_cooldown(update, context, seconds)
        elif data == "vcs:cdcustom":
            await vc_handler.enter_custom_cooldown(update, context)
        elif data.startswith("vcs:t:"):
            key = data.split(":", 2)[2]
            await vc_handler.toggle_setting(update, context, key)
        elif data == "vcs:refresh":
            await vc_handler.show_settings(update, context)
...
async def route_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif state == states.AWAIT_VC_COOLDOWN:
        await vc_handler.handle_custom_cooldown(update, context)
    else:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
